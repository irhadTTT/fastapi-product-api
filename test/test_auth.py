...
def test_login_success(client, db_session):
    user = User(
        username="test", email="test@test.com", password=hash_password("test123")
    )
    ...
    response = client.post(
        "/auth/login", data={"username": "test", "password": "test123"}
    )
...
def test_login_invalid_username(client):
    response = client.post(
        "/auth/login", data={"username": "test1", "password": "test1"}
    )
...
def test_login_invalid_password(client, db_session):
    ...
    response = client.post(
        "/auth/login", data={"username": "testwrong", "password": "wrongpassword123"}
    )
